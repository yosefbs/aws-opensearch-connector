    ...
    request(params, options = {}, callback = undefined) {
      ...
      if (typeof awsConfig.getCredentials !== 'function') {
        if (typeof callback === 'undefined') {
          return Promise.resolve(super.request(params, options));
        } else {
          Promise.resolve(super.request(params, options, callback));
        }
      } else {
        // Promise support
        if (typeof callback === 'undefined') {
          return awaitAwsCredentials(awsConfig).then(() =>
            Promise.resolve(super.request(params, options)),
          );
        }

        // Callback support
        awaitAwsCredentials(awsConfig)
          .then(() => Promise.resolve(super.request(params, options, callback)))
          .catch(callback);
      }
    }
  }
